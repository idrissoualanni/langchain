...
//   - sourceType "url"      → source de LIEN : <a> cliquable (nouvel onglet)
//   - sourceType "document" → source de RECHERCHE : badge non cliquable
//   - toute autre forme     → null (rien à afficher)
...
function SourceIcon({
  ...
  return (
    <img
      ...
      // Une image server-rendered qui échoue avant hydration ne déclenche
      // jamais onError → fallback via le ref à la complétion.
      ref={(el) => {
...
const SourcesImpl: SourceMessagePartComponent = (part) => {
  ...
  if (part.sourceType === "document") {
    return (
      <Badge className="focus-visible:border-ring focus-visible:ring-ring/50 outline-none focus-visible:ring-1">
