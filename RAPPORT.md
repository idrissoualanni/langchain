...
# ANNEXE HISTORIQUE — MISSION V5.2 (rapport intermédiaire original)

> Rapport V5.2 consolidé : le fichier `MISSION_V5.2_RAPPORT.md`
> a été fusionné ici puis supprimé — contenu historique conservé
> ci-dessous, avec notes de consolidation.

**Branche** : `mission/v5.2-pedagogical-tools` (créée depuis `master`)
**Statut au moment du rapport** : Backend ✅ fonctionnel et testé (55/58 assertions) — Frontend ⬜ non commencé — Commit ⬜ non fait.
...
Profile), une **exécution de code sandboxée** en sous-processus isolé (jamais dans le process
FastAPI principal), des **événements temps réel** (17 nouveaux types) et une **API d'activité**.
Tout est branché **sans toucher** `graph.py`, `runner.py`, `middleware.py`, `context/builder.py`,
`context/router.py`.

Mécanisme clé validé : les tools retournent `Command(update={...})` (langgraph) et lisent l'état
via `InjectedState` — l'écriture d'état se fait depuis les tools eux-mêmes, sans modifier le
...
| Contrainte | Respect |
|---|---|
| Ne pas modifier `graph.py`, `runner.py`, `middleware.py`, `context/builder.py`, `context/router.py` | ✅ aucun modifié |
| État d'activité dans Thread State | ✅ `learning_activity` (dict) + `activity_log` (reducer `operator.add`) + `code_runs` (int) dans `CustomAgentState` (`state.py`) |
...
- **`create_exercise(subject, topic)`** — construit l'exercice depuis la vraie section du cours
  (`app/knowledge/**/*.md`), détecte le type de réponse attendu (`code` si la matière déclare
  `execute_code`, sinon texte), passe l'activité en `waiting_for_answer`, instruit le LLM de
  poser la question PUIS attendre. Topic inconnu → liste des topics réels disponibles.
...
- **`give_hint(subject, topic, level)`** — **progressif strictement** : jamais de recul, avance
  de +1 max par appel. Niveaux 0/1/2 construits depuis le contenu réel du cours. Le saut direct
...
- **`create_quiz_next(...)`** — avance l'index après évaluation ; à la fin → `QUIZ_COMPLETED`
  avec score et bilan. (Quiz réutilise `learning_activity`, pas de champ dédié ; 2 tools au lieu
  de 3.)
- **`assess_understanding(subject, topic, response)`** — **observation, pas maîtrise** : statut
  `understood / partial / unclear / not_understood` selon couverture + longueur. **Aucune écriture
  Learning Profile** (test 52 le vérifie structurellement).
...
## E. Outils de pratique du code (sandbox)
...
- **`execute_code(language, code, subject)`** — garde : matière autorisée, python seul, taille
  ...
  sous-processus `python -I -B`, env purgé (`_safe_env` — **aucun secret serveur**, test s9),
  cwd répertoire jetable dédié sous `backend/database/code_runs`, timeout 10 s. Retour réel
  `{status, stdout, stderr, exit_code, duration_ms}`.
- **`run_tests(language, code, tests)`** — construit un harness (code étudiant + un `try/except`
  par test, résultats JSON), l'exécute isolé (15 s), parse et rapporte ✓/✗ par test avec
  obtenu/attendu.
- **`analyze_code(language, code)`** — `compile()` pour la syntaxe **sans exécuter**, règles
  ...
  structuré. **Le LLM ne réécrit JAMAIS le code de l'étudiant sans permission** (règles 24–26 du
  prompt + consignes dans les retours des tools).
...
`TemporaryDirectory` (TMP système interdit → racine déplacée sous `backend/database/code_runs`)
; schéma Pydantic rejetant `None` quand il n'y a pas d'activité (champs rendus nullables).
...
- `prompts.py` — règles **16–26** ajoutées (règle 15 conservée verbatim, test 52) : activité en
  cours, une question puis on attend, "Bonjour/Ok" ≠ réponse, clarification, hint progressif,
  vérification de compréhension, quiz conversationnel, multi-tentatives, pratique du code sans
  réécrire le code de l'étudiant.
...
- `tools.py` — import additif de `code_tools` et ajout à `all_tools` (24 tools au total), les
  learning tools intacts.

## I. Tests — résultats réels (au moment du rapport)
...
| §44 réponse → évaluation → compréhension | 3/4 (44b : calibrage) |
...
| §48 code multi-tentatives + run_tests + analyze_code | 4/5 (48d : bug run_tests) |
| §49 restart (nouvelle instance, même checkpointer) | 2/2 ✅ |
| §50 cross-thread (aucun transfert d'activité) | 3/4 (50c : calibrage) |
| §51 cross-user API (403 lecture, 403 run-code, 400 réseau, exécution réelle) | 5/5 ✅ |
| Sandbox sécurité | 12/12 ✅ |
| §52 agent 1 intact (4 tools learning + règle 15) | 5/5 ✅ |
| §38 indépendance matières (biologie fonctionne, pas de code) | 2/2 ✅ |

> Note de consolidation : le bug `run_tests` (comparaison repr/littéral) et les
> 2 calibrages (44b, 50c) ont été résolus par la suite — `test_v52_unit.py` est
> verte dans la régression actuelle (V7, 14/14 suites).

## J. Cohabitation — audit final

- **Modifications V5.2** : `pedagogical_tools.py` (réécrit), `state.py` (3 champs), `tools.py`
  (additif), `prompts.py` (règles 16–26), `schemas.py` (5 classes appendues), `main.py`
  (import + `include_router`), 4 YAML matières.
- **Fichiers V5.2** : `activity_state.py`, `code_tools.py`, `api/activity.py`,
  `test_v52_unit.py`.
- **Jamais touchés** : `learning_tools.py`, `app/learning/`, `api/learning.py`,
  `middleware.py`, `context/*`, `api/subjects.py`, tests V6, fichiers frontend learning.

## K. Ce qui restait à faire (au moment du rapport — depuis résolu)

1. Fix bug `run_tests` (comparaison repr) — **fait**.
2. Recalibrage 44b/50c — **fait**.
3. Frontend CodeEditor/TestResultPanel/ActivityFeed — **livré avec V5.2 final** (intégré au
   master).
4. Suppression scratch — **fait**.
5. Commit — **fait** (historique master consolidé).

## Arborescence réelle du projet (au moment du rapport)
...
backend/
  app/
    agent/
      ...
      tools.py               [V5.2 additif — +code_tools]
      prompts.py             [V5.2 additif — règles 16–26 ; V6 règle 15 conservée]
      graph.py  runner.py  middleware.py   [INTACTS]
      learning_tools.py     [AGENT 1 — intact]
    api/
      activity.py            [V5.2 NOUVEAU — GET activity / POST run-code]
      schemas.py             [V5.2 additif — 5 classes]
      main.py                [V5.2 additif — include_router(activity)]
    learning/                [AGENT 1 — intact]
    context/  db/  logging/  ws/  subjects/  config.py
    subjects/definitions/    [V5.2 MODIFIÉ — 4 YAML matières]
    knowledge/               [base de cours réelle, sections ## topic]
  tests/
    test_v52_unit.py         [V5.2 NOUVEAU — 58 assertions]
frontend/src/
  types/agent.ts  api/agent.ts  hooks/useChat.ts  components/chat/ToolStatusPanel.tsx
  api/learning.ts  types/learning.ts  components/memory/LearningProfileCard.tsx
```

**Conclusion historique** : le cœur backend V5.2 (tools interactifs, état d'activité persisté,
sandbox, API, événements, tests critiques) était opérationnel, prouvé par 55 assertions. Les
points restants (1 bug, 2 calibrages, frontend, commit) ont été résolus dans les versions
suivantes consolidées au master (V5.2 → V7).
