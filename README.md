...
Cette interface d'administration est accessible à l'URL **http://localhost:5173/admin**.

#### Variables d'environnement requises

- **ADMIN_CLERK_IDS** : liste d'IDs Clerk autorisant l’accès admin.
- **LIVEKIT_WS_URL** (ou **LIVEKIT_HOST**) : URL du serveur LiveKit (exemple `wss://localhost:7880`).
- **LIVEKIT_API_KEY** : clé API LiveKit.
- **LIVEKIT_API_SECRET** : secret API LiveKit.

#### Lancement du backend et du frontend pour l'admin UI

Le démarrage suit les étapes décrites précédemment : lancez d'abord le backend, puis le frontend. Une fois le serveur front‑end en cours d'exécution, accédez à `http://localhost:5173/admin` pour l'interface d'administration.
