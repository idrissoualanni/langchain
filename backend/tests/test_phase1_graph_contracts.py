...
# Phase 3 — détection d'intention "problem" (§21) : marqueurs
# déterministes (aucun LLM), sans casser la chaîne "main" par défaut
# ni la priorité de continuation d'activité.
check(
    "C: decide_workflow -> problem sur marqueur 'Résous'",
    decide_workflow({"query": "Résous 2x+3=7", "agent_config": {}}).workflow == "problem",
)
check(
    "C: decide_workflow -> problem sur marqueur 'Calcule'",
    decide_workflow({"query": "Calcule la dérivée de x²", "agent_config": {}}).workflow == "problem",
)
check(
    "C: decide_workflow -> main sans marqueur problem",
    decide_workflow({"query": "Explique-moi les fonctions Python.", "agent_config": {}}).workflow == "main",
)
check(
    "C: decide_workflow -> activity prioritaire sur problem",
    decide_workflow(
        {
            "query": "Résous 2x+3=7",
            "learning_activity": {"status": "waiting_for_answer", "activity_id": "a-7"},
        }
    ).workflow
    == "activity",
)
...
check(
    "C: route_after_workflow_router -> problem (Phase 3, câblé)",
...
from app.graph.nodes.problem import problem_node  # noqa: E402
...
graph.add_edge("problem", "context")
...
check(
    "D: compile_main_graph - 11 nodes presents (Phase 3: +problem)",
    len(node_names) >= 11,
