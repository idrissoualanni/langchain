# Subgraph Contracts (Phase 1) — contrats d'entrée/sortie des subgraphs (§8).
#
...
# interne au Main Graph). Ces contrats sont créés en Phase 1 pour
# fixer les frontières AVANT que les subgraphs ne soient implémentés
# (Phases 2-7) — typage strict extra=forbid, cohérent avec les
# contrats pydantic existants (RoutingResult, BuiltContext, ...).
...
# app.context.schemas ici (les contrats subgraph ne dépendent pas de
...
SUBGRAPH_RESULTS: dict[str, type[SubgraphResult]] = {
    ...
    "video": VideoResult,
    "document": SubgraphResult,  # DocumentSubgraph (§30) : sortie =
    # SubgraphResult générique tant que son contrat n'est pas affiné.
}

# Workflows de l'application (source de vérité du WorkflowRouter,
# Phase 1 : seul "main" est authentique à ce stade).
