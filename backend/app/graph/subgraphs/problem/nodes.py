...
def guide_node(state) -> dict:
    ...
    if not plan:
        return {"next_action": "evaluate_step"}
    ...
    return {
        "guide_message": guide,
        "next_action": "evaluate_step",
    }


# ------------------------------------------------------------
# EVALUATE_STEP node (boucle ↺ evaluate_step → guide)
# ------------------------------------------------------------
def evaluate_step_node(state) -> dict:
    """EVALUATE_STEP — évalue la soumission de l'étape courante (§21).

    La soumission vient du canal `submission` du state (saisie de
    l'étudiant à l'étape courante), sinon chaîne vide.

    Décision :
      - correct       → étape validée, current_index += 1
      - incorrect     → dernier mot au nombre de tentatives : on
                        avance quand même après MAX_ATTEMPTS (le
                        sous-graphe ne bloque pas la conversation).
    Routeur (route_after_evaluate_step) :
      - "validate" si fin du plan OU échec définitif
      - "guide" sinon (nouvelle tentative même étape)
    """
    plan = [dict(p) for p in (state or {}).get("plan") or []]
    current_index = int((state or {}).get("current_index") or 0)
    ...
    steps = _rehydrate_plan(plan)
    step_obj = steps[current_index] if current_index < len(steps) else steps[-1]

    attempts = int((state or {}).get("attempts") or 0) + 1
    submission = (state or {}).get("submission") or ""
    ...
    # Règle §19 : PAS de sanction sans preuve déterministe. Un verdict
    # "unclear" (aucune référence fournie → aucun score calculé) ne
    # bloque pas : on avance sans pénalité (l'agent conversationnel
    # guide l'étudiant). Une étape correcte est validée ; une étape
    # incorrecte (score déterministe < 0.4) déclenche un retry.
    if evaluation.verdict == "unclear" or evaluation.verdict == "correct":
        step_scores.append(
            {
        ...
        # On se met en attente de la soumission de l'étape suivante,
        # sauf si le plan est épuisé.
        return {
            ...
            "next_action": "validate" if next_index >= len(plan) else "guide",
        }

    # Incorrect (seulement si preuve déterministe du moteur §20) :
    # on retente (guide) jusqu'à MAX_ATTEMPTS puis valide.
    ...
    # Retry : nouvelle tentative sur la MÊME étape (boucle ↺).
    _log(
        "PROBLEM_STEP_RETRY",
...
def route_after_evaluate_step(state) -> str:
    ...
    L'état contient next_action posé par evaluate_step_node.
      - "validate" → node VALIDATE (plan épuisé / étape bloquée)
      - "guide"    → retour GUIDE (même étape, nouvelle tentative)
    """
    action = (state or {}).get("next_action") or "validate"
    if action == "guide":
        return "guide"
    return "validate"
