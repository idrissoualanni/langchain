...
def verify_admin_auth():
    ...
    En production, ceci doit vérifier le token Clerk et comparer
    avec ADMIN_CLERK_IDS. Pour l'instant, c'est un placeholder
    qui devra être connecté à l'authentification réelle.
    """
    # TODO: Intégrer la vérification Clerk réelle
    # Exemple : verifier le header Authorization, extraire le user_id,
    # et vérifier qu'il est dans settings.ADMIN_CLERK_IDS
    
    # Pour l'instant, on accepte si ADMIN_CLERK_IDS est configuré
    # Dans une vraie implémentation, il faudrait :
    # 1. Vérifier le token JWT Clerk
    # 2. Extraire le user_id
    # 3. Vérifier que user_id in settings.ADMIN_CLERK_IDS
    
    settings = get_settings()
    admin_ids = settings.ADMIN_CLERK_IDS or ""
    
    if not admin_ids:
        # Mode dev sans admin IDs configurés
        return {"user_id": "dev_admin", "is_admin": True}
    
    # Placeholder : en prod, vérifier le vrai user_id
...
async def get_run_detail(
    ...
    try:
        ...
        # Masquer les prompts systèmes complets
        if "system_prompt" in inputs:
            inputs["system_prompt"] = "[REDACTED]"
        ...
        # Récupérer les child runs
        child_runs = list(client.list_runs(
            project_name=run.project_name,
            reference_example_id=None,
            execution_order=2,  # Children only
        ))
        
        for child in child_runs:
            if child.parent_run_id == run.id:
                trace.child_runs.append(TraceDetail(
                    run_id=str(child.id),
                    ...
                    inputs={},
                    outputs={},
                    error=str(child.error) if child.error else None,
                    metadata={},
                    child_runs=[],
                ))
